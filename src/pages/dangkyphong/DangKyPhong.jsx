...
import RoomInfoCard from "../../components/radioCard/RoomInfoCard";
...
const DangKyPhong = () => {
  ...
  const filterListRooms = () => {
    ...
    if (inputValues.roomType) {
      newListRoom = [
        ...newListRoom.filter(
          (room) => room.numberOfBed === parseInt(inputValues.roomType)
        ),
      ];
    }
    if (parseInt(inputValues.gender) === 1) {
      newListRoom = [
        ...newListRoom.filter(
          (room) =>
            room.area === "A" ||
            (["4", "5"].includes(room.roomName.charAt(1)) && room.area === "B")
        ),
      ];
    }
    if (parseInt(inputValues.gender) === 0) {
      newListRoom = [
        ...newListRoom.filter(
          (room) =>
            room.area === "C" ||
            (["1", "2", "3"].includes(room.roomName.charAt(1)) &&
              room.area === "B")
        ),
      ];
    }
  ...
  const getListRoomType = () => {
    let listBedOfRoom = [
      ...new Set(listRoom.map((room) => room.numberOfBed).sort()),
    ];
    let newListRoomType = [];
        ...
        <div className="row row-cols-2 pt-5 ">
          ...
          >
            ...
            <Button
              ...
              onClick={() =>
                navigate(PATH.REGISTER.DANGKYTHONGTIN, {
                  state: { room: value },
                })
              }
